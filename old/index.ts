// TODO: "has to be a child of" option
interface IClassOptions {
	...
	match?: {
		...
		/**
		 * Unique CSS props to find the class name by.
		 *
		 * Note that sometimes the props are minified, i.e. the value for the
		 * `container` prop `modal-container/inline-size` works but
		 * `modal-container / inline-size` (attention to the spaces) doesn't.
		 *
		 * @todo only this works currently
		 * @todo can be really easily broken if 1 rule is removed, fallback ?
		 */
		cssProps?: Partial<Record<keyof CSSStyleDeclaration, string>>;

		/**
		 * Is the element a direct descendant of {@link childOf} ?
		 */
...
function RemoveElement<T>(vec: T[], what: T) {
	const index = vec.indexOf(what);
	if (index !== -1) {
		vec.splice(index, 1);
	}
}
(() => {
	...
	const result: Partial<Record<keyof typeof classMap, Record<string, string>>> =
		{};
	...
	function getNormalClass(className) {
		if (BLACKLISTED_CLASSES.some((e) => className === e)) {
			return;
		}
	...
	for (const component of components) {
		const classes: string[] = Object.values(result[component]);
